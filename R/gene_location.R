## code to prepare `gene_location` dataset goes here
generate_gene_location <- function(genes = NULL,
                                   regenerate = TRUE) {
  data("chromosome") #from this package
  if(regenerate == FALSE) {
...
gene_location <- generate_gene_location(regenerate = FALSE)
#gene_location <- gene_location %>% dplyr::rename(gene_name = name_1006) %>% select(!c(gene_biotype, source))

usethis::use_data(gene_location, overwrite = TRUE)
